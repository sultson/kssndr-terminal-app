package com.kssndrterminalapp2
...
class PsdkModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    @ReactMethod
    fun psdkSale(total: Double, promise: Promise) {
        try {
    ...
    fun psdkQueryLastTransaction(promise: Promise) {
        try {
            ...
            val transactionQuery = TransactionQuery.create()
    ...
    private val mCommerceListener = object : CommerceListener2(){
        ...
        override fun handlePaymentCompletedEvent(p0: PaymentCompletedEvent?) {
            ...
            Log.i(TAG, "In handlePaymentCompletedEvent " + p0?.message + " : " +
                    p0?.status + p0?.type)

            mPayment = paymentResult
            mAppSpecificData = paymentResult?.appSpecificData

//            builder.append("Presentation Method: ").append(cardInfo?.presentationMethod.toString()).append("\n")
//                .append("Payment Brand: ").append(cardInfo?.getPaymentBrand()).append("\n")
//                .append("Card Status: ").append(cardInfo?.getCardStatus()).append("\n")
//                .append("Account Type: ").append(cardInfo?.getAccountType().toString()).append("\n")
//                .append("Card Token: ").append(cardInfo?.cardToken).append("\n")
//                .append("Account Reference: ").append(cardInfo?.getAccountReference()).append("\n")
//
//
//            for ((key, value) in cardInfo!!.emvTags)
//                builder.append(key).append(": ").append(value).append("\n")
//
//            Log.i(TAG, builder.toString());
            val data = Arguments.createMap().apply {
                putString("payment", paymentResult.toString())
            }
            val params = Arguments.createMap().apply {
                putString("status", p0?.status.toString());
                putString("message", p0?.message);
...
//                    }
                    ...
                    val data = Arguments.createMap().apply {
                        putString("lastPayment", payments[0].toString())
                    }
                    val params = Arguments.createMap().apply {
                        putString("status", p0?.status.toString());
                        putString("message", p0?.message);
